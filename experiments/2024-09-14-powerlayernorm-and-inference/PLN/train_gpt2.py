...
class PowerLayerNorm(nn.Module): # warmup_iters set to 10% of total iterations
    def __init__(self, num_features, eps=1e-5, alpha_fwd=0.9, alpha_bkw=0.9, warmup_iters=2000, process_group=None):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.training:
            self.accum_count += 1   # to make sure that we are not adding steps each time layer is ran. Since when samples_per_step==8, 8 forward passes is done per iter
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.ln_1 = PowerLayerNorm(config.n_embd)
        self.attn = CausalSelfAttention(config)
        self.ln_2 = PowerLayerNorm(config.n_embd)
        self.mlp = MLP(config)
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            ln_f = PowerLayerNorm(config.n_embd),
        ))
...
# logging powerLayerNorm stats
def log_pln_stats(model):
    stats = {}
    for name, module in model.named_modules():
        if isinstance(module, PowerLayerNorm):
            stats[f"{name}/x_LN-weights"] = (module.alpha + module.eps).item()
            stats[f"{name}/x_PN-weights"] = (1 - (module.alpha + module.eps)).item()
            ...
            # Gradient of alpha, if you want to track how it's changing
            if module.alpha.grad is not None:
                stats[f"{name}/alpha_grad"] = module.alpha.grad.item()
    return stats
...
config = {
    ...
    "project_name": "shng2025/GPT-Valkyrie_PLN-124m",
    ...
    "warmup_steps": 715,
    "max_steps": 21798,     # this had been scaled up by 12.5% from 19073, since we had reduced the number of tokens for micro_batch_size
...
for step in range(starting_step, max_steps):
    ...
    if master_process:
        pln_stats = log_pln_stats(raw_model)
        log_metrics({
            "lr": lr, # get_lr()
